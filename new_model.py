...
def key_generation_user(Us):
    ...
    for user in user_list:
        ...
        for idx,val in enumerate(Us_user.keys()):
            ...
            #storing the shares of user secret key in system
            shares=[]
            temp=str(Us_user[val]).encode('UTF-8')
            ...
            while(idx<len(temp)):   
                ...
                shares.append(random.sample(blocks,t))
                idx+=16
            ...
            Us_user[val]=(shares,padding_bit)
        Us[user]=Us_user
...
def calculate_shares(DU,Us):
    ...
    for idx,val in enumerate(Us[DU].keys()):
        #combine the shares
        temp,padding_yes=Us[DU][val]
        user_secret=b''

        for idx in range(len(temp)):
            block=temp[idx]
            temp_secret=Shamir.combine(block)
            user_secret+=temp_secret
        
        if padding_yes==1:
            unpadder = padding.PKCS7(128).unpadder()
            user_secret= unpadder.update(user_secret) + unpadder.finalize() 
        reconstructed_us[val]=user_secret
    ...
    return (reconstructed_us,t*1000)
...
def calc(Message,access_policy):
    ...
    C,C0,C1,C2,CH,Qd,shares,a,w,no_of_attributes,t3=encrypt_DO(Message,access_policy,pk)
    encryption_time=t3
    ...
    User_secret,t6=calculate_shares(DU,Us)
    D,N1,N2,C,ok,t5=decryption_DUA(DU,C,C0,C1,C2,CH,User_secret,shares,access_policy,a,w)
    ok,M,t7=decryption_DU(DU,N1,N2,C,C0,CH,P[DU],Qd,ok)   
    if (ok==True):
        # print("\n same",M==Message)
...
encrypted_path='/home/anika/Desktop/Thesis/Image_data/Encrypted_5/'
path_to_write='/home/anika/Desktop/Thesis/Image_data/Decrypted_5/'
