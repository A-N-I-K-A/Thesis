...
def registration_user_phase(user_no):
    ...
    #a random redundancy is selected
    #redundancy= G*R
    R=group.random(ZR)
...
def registration_CA(GID,Ru,user_no,R):
...
def calculate_shares(C,DU):
    t=0
    group.InitBenchmark( )
    group.StartBenchmark(["RealTime"])


    #divide cipher text into n shares and set the threshold to m
    ...
    while(idx<len(C)):
        ...
        shares_of_cipher.append(block)
        idx+=16


    
    shares=[]
    for block in shares_of_cipher:
        shares.append(random.sample(block,m))

    #divide the redundancy into shares 
    R=redundancy[DU]
    n=len(R)//16
    m=n//2
    idx=0
    shares_of_redundancy=[]
    while(idx<len(R)):
        temp=R[idx:idx+16]
        if (len(temp)<16):
            padder = padding.PKCS7(128).padder()
            temp= padder.update(temp) + padder.finalize()
        block=Shamir.split(m,n,temp)
        shares_of_redundancy.append(block)
        idx+=16

    #append the redundancy shares into cipher text share for deception
    for block in shares_of_redundancy:
        shares.append(random.sample(block,m))


    if debug==True:
        print("\nn",n)
        print("\nshares of cipher",shares_of_cipher)
        print("\nshares of redundancy",shares_of_redundancy)
        print("\nshares",shares)
...
def decryption_DU(DU,N1,N2,C0,CH,p,Qd,shares,ok):
    ...
    if(ok==False):
        ...
        R=redundancy[DU]
        combines=[]
        for blocks in shares:
            combines.append(Shamir.combine(blocks))

 
        C=b''
        R1=b''
        for blocks in combines:
            #filter out the redundant shares
            if blocks not in (R):
                C+=blocks
            else:
                R1+=blocks
...
def calc(Message,access_policy):
    ...
    for user_no,user in enumerate(user_list):
        ...
        (Ru,GID,R)=registration_user_phase(user_no)
        (Cu,Pc,CP)=registration_CA(GID,Ru,user_no,R)
    ...
    shares,t6=calculate_shares(C,DU)
    ok,M,t7=decryption_DU(DU,N1,N2,C0,CH,P[DU],Qd,shares,ok)   
    if ok==True:
        # print("\nMessage",Message)
        # print("\n",M)
