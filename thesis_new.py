...
def decryption_DUA(DU,C,C0,C1,C2,CH,Us,c1,shares,access_policy,a,w):
    ...
    #         temp1=a[x][idx]*cx[x][idx]*G
    #         temp2=w[x][idx]*Hash_gid*cx[x][idx]
    #         temp2*=G
    #         temp3=p*cx[x][idx]*G
    #         temp=temp1+temp2-temp3
      
         
    #         #comp=((a[x][idx]+Hash_gid*w[x][idx]-p)*cx[x][idx])
    #         N1+=(temp)
    #         temp_arr.append(temp)
...
def calc(Message,access_policy):
    ...
    C,C0,C1,C2,CH,Qd,shares,a,w,no_of_attributes,t3=encrypt_DO(Message.encode('UTF-8'),access_policy,msk)
