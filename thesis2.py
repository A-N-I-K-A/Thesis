...
def reencrypt(C0,C1,C2,c,shares,a,w,C,attr_list):

    t=0
    group.InitBenchmark( )
    group.StartBenchmark(["RealTime"])

    new_c1= os.urandom(32)
    reencryption_key=group.random(ZR)
    key[ reencryption_key]=new_c1
    ...
    c1 = os.urandom(32)
    key[c]=c1
    cipher = Cipher(algorithms.AES(key[reencryption_key]), modes.CBC(iv),default_backend())
    encryptor = cipher.encryptor()
    C = encryptor.update(padded_data) + encryptor.finalize()
    ...
    return (C0,C1,C2,c,shares,a,w,C,attr_list,reencryption_key,t*1000)
...
def decryption(N1,N2,C0,C1,C2,p,C_r,ok,reencryption_key):
    ...
    if(ok==False):
        ...
        C= decryptor.update(C_r)+decryptor.finalize()
...
def main():
    ...
    C0,C1,C2,c,shares,a,w,C_r,attr_list,reencryption_key,t4=reencrypt(C0,C1,C2,c,shares,a,w,C,attr_list)
    print("\nEncryption time in ms",t3+t4)

    D,N1,N2,cx,pruned_list,c,ok,t5=decryption_DUA(C0,C1,C2,Us,c,shares,p,DU,access_policy[DO],a,w,attr_list,msk)
    ok,M,t6=decryption(N1,N2,C0,C1,C2,p,C_r,ok,reencryption_key)   
    if ok==True:
